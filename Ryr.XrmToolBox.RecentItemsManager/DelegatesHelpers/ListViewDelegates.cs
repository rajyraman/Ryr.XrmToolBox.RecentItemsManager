...
namespace Ryr.XrmToolBox.RecentItemsManager
{
    public class ListViewDelegates
    {
        ...
        public static ListViewItem[] GetItems(ListView listview)
        {
            ListViewItem[] items = new ListViewItem[0];

            MethodInvoker miGetItems = delegate
            {
                items = new ListViewItem[listview.Items.Count];
                listview.Items.CopyTo(items, 0);
            };

            if (listview.InvokeRequired)
            {
                listview.Invoke(miGetItems);
            }
            else
            {
                miGetItems();
